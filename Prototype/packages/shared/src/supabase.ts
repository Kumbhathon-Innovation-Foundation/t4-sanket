...
const envSupabaseUrl = metaEnv ? (metaEnv.VITE_SUPABASE_URL as string) : undefined;
const envAnonKey = metaEnv ? (metaEnv.VITE_SUPABASE_ANON_KEY as string) : undefined;
...
export const supabaseAnon: SupabaseClient | null = isSupabaseClientConfigured
...
export function subscribeToRouteUpdates(
  onUpdate: (route: RouteStatus) => void,
  onReset?: () => void
): () => void {
  ...
  if (supabaseAnon) {
    try {
      const channel = supabaseAnon
        ...
        .on(
          'postgres_changes',
          { event: '*', schema: 'public', table: 'route_status' },
          (payload) => {
            if (payload.new && (payload.new as any).route_id) {
              const updatedRecord = payload.new as any;
              const formattedRoute: RouteStatus = {
                ...
                tier: updatedRecord.tier as any,
                crowd: updatedRecord.crowd as any,
  ...
  if (typeof window !== 'undefined' && 'BroadcastChannel' in window) {
    try {
      ...
      const handler = (ev: MessageEvent<RealtimeEvent>) => {
 ...
 */
export function broadcastLocalEvent(event: RealtimeEvent): void {
  if (typeof window !== 'undefined' && 'BroadcastChannel' in window) {
    try {
      const bc = new BroadcastChannel(BROADCAST_CHANNEL_NAME);
